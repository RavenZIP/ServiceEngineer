...
import com.example.serviceengineer.screens.helpers.programsList
...
/*    Button(onClick = {
        val pushKey = databasePrograms.push().key.toString()
        val paper = Paper(
            paperId = pushKey,
            paperName = "AIDA64",
            paperDescription = "",
            paperText = "text",
            paperImages = listOf("1", "2")
        )
        databasePrograms.child(pushKey).setValue(paper)
    }) {
        Text(text = "tttt")
    }*/
}